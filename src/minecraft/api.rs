...
use axum::{body, extract::{Path, Query}, http::{header, Response}, response::IntoResponse, routing::get, Json, Router};
use serde::{Serialize, Deserialize};
use crate::{status_403_handler, status_404_handler};
use tokio::{fs::{File, metadata}, io::AsyncReadExt, join};
use std::fs::read_dir;
use reqwest;
...
async fn server_route_handler(Path(identifier): Path<String>, headers: header::HeaderMap) -> impl IntoResponse {
    let mut guilds: Vec<String> = Vec::new();
    if headers.get("guilds").is_some_and(|value| value != "null") {
        guilds = serde_json::from_str::<Vec<String>>(
            headers.get("guilds").unwrap().to_str().unwrap()
        ).unwrap()
    }
...
async fn server_icon_route_handler(Path(server_id): Path<String>) -> impl IntoResponse {
    ...
    let req = client.get(format!("http://{}:{}/api/v2/servers", dotenv!("INTERNAL_IP"), dotenv!("MCSS_PORT")))
        .header("apiKey", dotenv!("MCSS_KEY"))
        .send()
        .await.unwrap().json::<Vec<McssServer>>();

    let server =  match req.await {
        Ok(server) =>  match server.into_iter().find(|server| server.server_id == server_id) {
            Some(server) => server,
    ...
    Response::builder()
        ...
        .header(header::CONTENT_DISPOSITION, "attachment; filename=\"server-icon-256.png\"")
        .body(body).unwrap().into_response()
...
async fn server_map_route_handler(Path(server_id): Path<String>) -> impl IntoResponse {
    ...
    let paths = match read_dir(format!("{}map", server.path_to_folder)) {
        Ok(paths) => paths.map(|dir| dir.unwrap().path().file_stem().unwrap().to_str().unwrap().to_string().replace("-", ":"))
            .collect::<Vec<_>>(),
        Err(_) => return status_404_handler().await.into_response()
    };
...
async fn server_world_route_handler(Path(server_id): Path<String>) -> impl IntoResponse {
    ...
    Response::builder()
        ...
        .header(header::CONTENT_DISPOSITION, &format!("attachment; filename=\"{}_world.zip\"", server.folder_name))
        .body(body).unwrap().into_response()
...
pub async fn get_minecraft_servers(sorting_options: Query<SortingOptions>) -> Vec<ServerData> {
    ...
    let req = client.get(format!("http://{}:{}/api/v2/servers", dotenv!("INTERNAL_IP"), dotenv!("MCSS_PORT")))
        ...
        .send()
        .await.unwrap().json::<Vec<McssServer>>();  
    let servers = req.await.unwrap().into_iter().map(server_mapper).collect::<Vec<_>>();
    let mut awaited_servers: Vec<ServerData> = Vec::new();
    for server in servers {
        awaited_servers.push(server.await);
    }
    // Sort
    if sorting_options.sort.as_deref().is_some_and(|sort| sort == "asc") {
        awaited_servers.sort_by(|a, b| a.name.to_lowercase().cmp(&b.name.to_lowercase()));
    }
    if sorting_options.sort.as_deref().is_some_and(|sort| sort == "desc") {
        awaited_servers.sort_by(|a, b| b.name.to_lowercase().cmp(&a.name.to_lowercase()));
    }
    if sorting_options.sort.as_deref().is_none() || sorting_options.sort.as_deref().is_some_and(|sort| sort == "default" || sort == "na"){
        awaited_servers.sort_by(|a, b| a.cmp(b));
    }
    // Filter
    if sorting_options.filter.as_deref().is_some_and(|filter| filter == "deleted") {
        awaited_servers = awaited_servers.into_iter().filter(|server| server.status == 0).collect::<Vec<_>>();
    }
    if sorting_options.filter.as_deref().is_some_and(|filter| filter == "current") {
        awaited_servers = awaited_servers.into_iter().filter(|server| server.status == 1).collect::<Vec<_>>();
    }
    if sorting_options.filter.as_deref().is_some_and(|filter| filter == "archived") {
        awaited_servers = awaited_servers.into_iter().filter(|server| server.status == 2).collect::<Vec<_>>();
    }
    awaited_servers
}

pub async fn server_mapper(server: McssServer) -> ServerData {
    let server_description = serde_json::from_str::<ServerDescription>(&server.description).unwrap();
    let mut online = false;
    if server.status == 1 { online = true }
    let mut server_status = 1; // default is current
    if server_description.status == "archived" { server_status = 2 }
    if server_description.status == "deleted" { server_status = 0 }
    ...
    if file.is_err() {
        return ServerData {
            ...
            server_dates: ServerDates { start: "Invalid".to_string(), end: "Invalid".to_string() },
            link: ServerLink { t: "Invalid".to_string(), url: "Invalid".to_string() }
        }
    } 
    ...
    if web_config.pack.url.contains("modrinth") { link_type = "modrinth"}
    let mut link = web_config.pack.url;
    if link_type == "curseforge" { link = format!("curseforge://install?addonId={}&fileId={}/", web_config.pack.id, web_config.pack.file_id)}
    ...
    ServerData {
        ...
        link: ServerLink { t: link_type.to_string(), url: link }
    }
}

pub async fn get_minecraft_server(identifier: String, guilds: Vec<String>, ip: String) -> ExtendedServerData {
    let servers = get_minecraft_servers(Query(SortingOptions { filter: None, sort: None })).await;
    let server = match servers.into_iter().find(|server| server.identifier == identifier) {
      Some(server) => server,
      None => return ExtendedServerData {
        id: "".to_string(),
        status: 0,
        online: false,
        version: "".to_string(),
        ...
        server_dates: ServerDates { start: "".to_string(), end: "".to_string()},
        link: ServerLink { t: "".to_string(), url: "".to_string() },
        ip: None,
        players: None,
        world_download: None,
    ...
    let (server_req, stats_req) = join!(
        client.get(format!("http://{}:{}/api/v2/servers/{}", dotenv!("INTERNAL_IP"), dotenv!("MCSS_PORT"), server.id))
            .header("apiKey", dotenv!("MCSS_KEY"))
            .send()
            .await.unwrap().json::<McssServer>(),
        client.get(format!("http://{}:{}/api/v2/servers/{}/stats", dotenv!("INTERNAL_IP"), dotenv!("MCSS_PORT"), server.id))
            .header("apiKey", dotenv!("MCSS_KEY"))
            .send()
            .await.unwrap().json::<McssServerStats>()
    );

    let server_req_clone = server_req.iter().clone().collect::<Vec<_>>();
    let server_req = server_req_clone.first().unwrap();

    let server_description = serde_json::from_str::<ServerDescription>(&server_req.description).unwrap();
    ...
    ExtendedServerData {
        ...
        world_download: Some(metadata(format!("{}world.zip", server_req.path_to_folder)).await.is_ok()),
        internal_pack_download: Some(metadata(format!("{}pack.zip", server_req.path_to_folder)).await.is_ok())
    }
}
