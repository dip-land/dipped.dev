...
export async function sendHtml(res: FastifyReply, file: string) {
    res.type('text/html').send(createReadStream(path.join(process.cwd(), file)));
    return res;
}
...
export async function constructPage(
    ...
    options: {
        language: string;
        head: { url?: string; title?: string; description?: string; keywords?: string; image?: string; video?: string; largeMedia?: boolean; files?: Array<string> };
        body: { files?: Array<string> };
    },
    document?: (window: DOMWindow, document: Document) => Promise<any>
) {
    ...
    try {
        ...
        res.type('text/html').send(dom.serialize());
    } catch (error) {
        console.log(error);
...
export function isVideo(filePath: string | undefined) {
