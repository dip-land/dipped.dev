...
export default async (interaction: UserContextMenuCommandInteraction) => {
    ...
    if (!cmd || cmd.disabled || !cmd.contextUserCommand) return interaction.reply({ content: 'This command is disabled, it may be re-enabled in the future.', ephemeral: true });
    ...
    if (client.isBotOwner(interaction.user)) return cmd?.contextUserCommand({ interaction, options: interaction.options, client });
    ...
    cmd.contextUserCommand({ interaction, options: interaction.options, client }).catch((err) => client.error(err));
};
