...
export default new Command({
    ...
    async slashCommand({ interaction, client }) {
        ...
        if (options.data[0].name === 'create' || options.data[0].name === 'edit') {
            ...
            collection.findOneAndUpdate({ userID: interaction.member?.user.id }, { $set: { name, color } }, { upsert: true }).then(async () => {
                ...
                if (document!.roleID) {
                    interaction.guild?.roles.fetch(document!.roleID).then((role) => {
                    ...
                    interaction.guild?.roles.create({ name, color }).then(async (role) => {
