...
use templates::{
    head, main as template_main, main_section, r6_nav,
    status_r6::status_404_handler,
...
async fn main() {
    ...
    let mut port = dotenv!("R6_WEB_PORT");
    ...
    let assets_path = Path::new(dotenv!("R6_ASSETS_PATH"));
...
fn debug_fn() {
    ...
    for entry in glob(format!("{}/**/*.scss", dotenv!("R6_ASSETS_PATH")).as_str())
        .expect("Failed to read glob pattern")
    {
        if entry.is_err() {
            continue;
