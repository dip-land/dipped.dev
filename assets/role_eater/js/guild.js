...
window.addEventListener('load', async () => {
    ...
    const guildID = window.location.pathname.replace('/dashboard/', '');
    const data = await (await fetch(`/api/${guildID}`, { headers: { guilds: localStorage.getItem('guilds') } })).json();
    const chart_data = (await (await fetch(`/api/${guildID}/activity`, { headers: { guilds: localStorage.getItem('guilds') } })).json()).data;
