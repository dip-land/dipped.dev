...
let random_attacker_list = shuffle(JSON.parse(localStorage.getItem("attackers")));
let random_defender_list = shuffle(JSON.parse(localStorage.getItem("defenders")));
...
for (const button of buttons) {
    ...
    button.addEventListener('click', () => {
        ...
        if (href === "random_attacker") {
            if (random_attacker_list.length < 2) random_attacker_list = shuffle(JSON.parse(localStorage.getItem("attackers")));
            let randomIndex = Math.floor(Math.random() * random_attacker_list.length);
            let selected = random_attacker_list[randomIndex];
            random_attacker_list.splice(randomIndex, 1);
            set_selected(selected);
        }
        if (href === "random_defender") {
            if (random_defender_list.length < 2) random_defender_list = shuffle(JSON.parse(localStorage.getItem("defenders")));
            let randomIndex = Math.floor(Math.random() * random_defender_list.length);
            let selected = random_defender_list[randomIndex];
            random_defender_list.splice(randomIndex, 1);
            set_selected(selected);
        }
    })
}
...
function set_selected(name) {
    let selected = all_operators.find((v) => v.name === name);
    selected_element.querySelector("img").src = selected.image_src;
    selected_element.querySelector("span").innerText = selected.name;
}
