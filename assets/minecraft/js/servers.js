window.addEventListener("load", async () => {
	...
	if (document.getElementById("map_frame")) {
		document.getElementById("map_frame").src =
			`/maps/${server.identifier}/minecraft-overworld/index.html`;

		document
			.getElementById("selected_dimension")
			.addEventListener("change", (event) => {
				document.getElementById("map_frame").src =
					`/maps/${server.identifier}/${event.target.value}/index.html`;
			});
	}
	...
	try {
		const availableMaps = await (
			await fetch(`/api/maps/${server.id}`, { headers: { guilds } })
		).json();
